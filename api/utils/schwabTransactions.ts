 ...
 *
 * NOTE: This is a copy for API serverless functions. Keep in sync with src/utils/schwabTransactions.ts
 */
...
export const mapSchwabTransactionsToTrades = (transactions: SchwabTransaction[]): Trade[] => {
    ...
    if (orphanedTrades.length > 0) {
        ...
        console.warn(
            `\n[Schwab Mapper] SYNC INCOMPLETE: ${orphanedTrades.length} orphaned closing trades detected.`,
